...
class ResearchAgentState(TypedDict):
    ...
    # 处理状态，Optional[Dict[str, Any]] 表示该变量 可以是 `Dict[str, Any]` 类型，也可以是 `None`(很好理解：即optional可选择性的)
...
def perception(state: ResearchAgentState) -> ResearchAgentState:
    ...
    try:
        ...
        return {
            **state,
            "perception_data": result,  #下一个节点会先判断perception_data是不是空的
    ...
    try:
        ...
        return {
            **state,
            ...
            "current_phase": "decision"  #下一个节点是决策阶段，更新到state中
        }
    except Exception as e:
        return {
            **state,
...
def decision(state: ResearchAgentState) -> ResearchAgentState:
    ...
    try:
        ...
        input_data = {
            ...
            "industry_focus": state["industry_focus"],
            "time_horizon": state["time_horizon"],
            "world_model": json.dumps(state["world_model"], ensure_ascii=False, indent=2), #这里居然带了前面两个节点
...
def create_research_agent_workflow() -> StateGraph:
    ...
    # 添加节点
    workflow.add_node("perception", perception)  #把五个函数(即节点定义)添加到工作流中
...
if __name__ == "__main__":
    ...
    if industry.strip().lower() == "exit":
        print("感谢使用，再见！")
    horizon = input("请输入时间范围 [短期/中期/长期]: ")
    if horizon.strip().lower() == "exit":
        print("感谢使用，再见！")
        sys.exit()
